...
export function useVerificationRouter() {
  ...
  const selectAndRoute = async (method) => {
    // 1. Remember in localStorage for instant preselection upon next login
    localStorage.setItem('last_verification_method', method);

    // 2. Log selection to backend if online
    try {
      await apiClient.post('/verification-method/select', {
        selected_method: method,
        user_id: 'user-01',
        timestamp: new Date().toISOString()
      });
    } catch (err) {
      console.warn('Backend method select log skipped:', err);
    }

    // 3. Route to target verification flow
    if (method === 'payslip') {
